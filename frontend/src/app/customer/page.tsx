                  ...
                  {pastBookings.length === 0 ? (
                    ...
                    pastBookings.map((booking: any) => (
                      <div key={booking.id} className="border border-border rounded-2xl bg-card p-6 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:border-primary/50 transition-colors">
                        ...
                        {booking.status === 'ACCEPTED' && booking.otp_code && (
                          <div className="text-right">
                             <p className="text-xs text-muted-foreground mb-1 font-bold uppercase tracking-wider">Closure OTP</p>
                             <p className="text-2xl font-mono tracking-widest font-black text-foreground">{booking.otp_code}</p>
                          </div>
                        )}
                        {['ACCEPTED', 'ARRIVED', 'IN_PROGRESS'].includes(booking.status) && (
                          <div className="text-right ml-4">
                            <button 
                              ...
                              className="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-xl transition-colors shadow-sm flex items-center justify-center gap-2 text-sm"
                            >

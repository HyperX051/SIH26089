...
export default function AdminApp() {
  ...
  const StatCard = ({ label, value, loading }: { label: string; value: string | number | null; loading: boolean }) => (
    <div className="bg-white p-6 border border-zinc-200 shadow-[4px_4px_0px_rgba(0,0,0,1)]">
      ...
      {loading ? (
        ...
        <h3 className="text-2xl font-bold text-zinc-400">No data</h3>
      )}
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col font-sans text-zinc-900">
      ...
      {/* Top Navbar */}
      <header className="bg-white text-zinc-900 px-8 h-16 flex justify-between items-center border-b border-zinc-200 sticky top-0 z-50">
        <Link href="/" className="flex items-center gap-3 cursor-pointer">
          ...
          <div>
            <h1 className="text-xl font-extrabold text-zinc-900 tracking-tight uppercase">Admin</h1>
          </div>
        </Link>
        ...
        <div className="lg:col-span-4 space-y-8">
              ...
              {loadingLedger ? (
                ...
                <>
                  <div className="bg-zinc-900 p-5 border border-zinc-800">
                    <p className="text-zinc-400 text-xs font-bold uppercase tracking-wider mb-2">Gross Turnover</p>
          ...
          <section className="bg-white border border-zinc-200 p-8">
            <div className="flex justify-between items-center mb-6">
              ...
              {kycPending.length > 0 && (
                <span className="bg-black text-white text-[10px] font-bold px-3 py-1 uppercase tracking-widest">{kycPending.length} Pending</span>
              )}
            </div>
            {kycPending.length === 0 ? (
